...
def run_pipeline():
    ...
    t2 = BashOperator(
        ...
        bash_command="python3.12 ${AIRFLOW_HOME}/future_sales_prediction/prepare_data.py "+f"--run_name {run_name} --source_path  {data_cleaned_path} --destination_path  {data_path} "
    )
    ...
    t4 = BashOperator(
        ...
        bash_command="python3.12 ${AIRFLOW_HOME}/future_sales_prediction/create_submission.py " + \
            f"--run_name {run_name} --path_for_merged {data_path} --path_data_cleaned  {data_cleaned_path} --is_create_submission 0 --batch_size_for_train {batch_size_for_train}"
    )
    t5 = BashOperator(
        task_id="create_submission",
        bash_command="python3.12 ${AIRFLOW_HOME}/future_sales_prediction/create_submission.py " + \
            f"--run_name {run_name} --path_for_merged {data_path} --path_data_cleaned  {data_cleaned_path} --is_create_submission 1 --batch_size_for_train {batch_size_for_train}"
    )

    t6 = BashOperator(
        task_id="error_analysis",
        bash_command="python3.12 ${AIRFLOW_HOME}/future_sales_prediction/error_analysis.py " +  \
            f"--run_name {run_name} --path_for_merged {data_path} --path_data_cleaned {data_cleaned_path} --path_artifact_storage {images_storage_path} "
    )
    ...
    t7 = BashOperator(
        ...
        bash_command="python3.12 ${AIRFLOW_HOME}/future_sales_prediction/model_interpret.py " + \
            f"--run_name {run_name} --path_for_merged {data_path} --path_data_cleaned {data_cleaned_path} --path_artifact_storage {images_storage_path} --batch_size_for_train {batch_size_for_train} "
    )
