...
def merge_boosting(item_shops, names, chunksize=None,item_shop_city_category_sup_category=None,destination_path=None):
    """
    ...
    Args:
        ...
        destination_path (str): path to the folder to write merged.csv in 
    """
    def create_batch_for_writing(file_length, chunksize):
        ...
        Returns:
            list[list[int]]: indexes for batches in files from panamesthes
        """
        l = file_length
    ...
    for idx in idxs:
        ...
        for path in names:
            ...
            batch=pd.read_csv(f'{destination_path}/{path}.csv',skiprows=range(1,idx[0] + 1),nrows=idx[1] - idx[0])
            if first_inner:
        ...
        if first:
            ...
            init.merge(item_shop_city_category_sup_category,on=['shop_id','item_id'], how='left').to_csv(f'{destination_path}/merged.csv', index=False)
            print('init memory usage,',sum(init.memory_usage()/10**6))
            first= False
        else:
            init.merge(item_shop_city_category_sup_category,on=['shop_id','item_id'], how='left').to_csv(f'{destination_path}/merged.csv', mode='a', index=False, header=False)
...
def prepare_files(data_train, item_shop_city_category_sup_category, alpha=2/(6+1),destination_path=None):
    ...
    Args:
        ...
        item_shop_city_category_sup_category (pd.DataFrame): Datarame with unique [shop, item] pairs. Used to return data with all requierd [shop, item] pairs.
        alpha (float): EMA smoothing factor.
        destination_path(str): folder to write temporary files
...
def calc_and_write_chunk(data_train,item_shop_city_category_sup_category, chunksize_when_writing,destination_path):
    ...
    if prepare_for_boosting:
        merge_boosting(shop_item, names, chunksize_when_writing,item_shop_city_category_sup_category,destination_path)
    t2=time.time()
...
def run_prepare_data(source_path, destination_path):
    ...
    Args:
        source_path (str): path to the folder where etl.py writes it's result
        destination_path (str): path to the folder where to store merged.csv
    """
    total_time1 = time.time()
    data_train = pd.read_csv(f'{source_path}/data_train.csv')
    item_categories = pd.read_csv(f'{source_path}/item_categories.csv')
