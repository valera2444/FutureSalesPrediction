...
def train_model(model, batch_size, val_month, shop_item_pairs_WITH_PREV_in_dbn,batch_size_to_read,epochs):
    ...
    for epoch in range(epochs):
        for X_train,Y_train  in create_batch_train(batch_size, val_month,shop_item_pairs_WITH_PREV_in_dbn,batch_size_to_read):
            ...
            elif type(model) == xgb.XGBRegressor or type(model) == xgb.XGBRFRegressor:
                if first:
...
if __name__ == '__main__':
    ...
    start_val_month=22
    #verbose=-1,n_jobs=8, num_leaves=256, n_estimators = 500, learning_rate=0.005
    #'lr': 0.006879070737977554, 'num_leaves': 32, 'n_estimators': 900
    ...
    #model = xgb.XGBRFRegressor(max_depth=12,device='gpu', enable_categorical=False,n_estimators=350)
    batch_size=70000000
